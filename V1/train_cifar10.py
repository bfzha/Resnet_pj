...
parser.add_argument('--nowandb', action='store_true', help='disable wandb 禁用Weights & Biases实验跟踪，默认是启用的')
...
if usewandb:
    import wandb
    watermark = f"{args.net}_lr{args.lr}_{args.dataset}"
...
transform_train = transforms.Compose([
    transforms.RandomCrop(32, padding=4),#随机裁剪变换：先在图像周围填充4个像素，然后随机裁剪出32x32大小的图像
    transforms.Resize(size),
    transforms.RandomHorizontalFlip(),#随机水平翻转图像，概率为0.5，这是另一种数据增强技术，有助于提高模型泛化能力
    transforms.ToTensor(),
...
# Add RandAugment with N, M(hyperparameter) 当数据增强时，使用RandAugment  N: 应用的数据增强操作数量（每次随机选择N个增强操作）M: 增强强度参数（控制增强的程度）
...
if args.resume:
    ...
    checkpoint_path = f'./checkpoint/{args.net}-{args.dataset}-{args.patch}-ckpt.t7'
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, f'./checkpoint/{args.net}-{args.dataset}-{args.patch}-ckpt.t7')
        best_acc = acc
...
# writeout wandb
if usewandb:
    wandb.save(f"wandb_{args.net}_{args.dataset}.h5")
